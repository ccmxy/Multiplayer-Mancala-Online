<?php
function setEndZone1($player_1_id){
  $mysqli = new mysqli("oniddb.cws.oregonstate.edu","minorc-db","kLdt7swbvLhRmjzu","minorc-db");
  if($mysqli->connect_errno){
  	echo "Connection error " . $mysqli->connect_errno . " " . $mysqli->connect_error;
  	}

if(!($stmt = $mysqli->prepare("SELECT endzones.seedCount FROM endzones WHERE endzones.id=$player_1_id"))){
  echo "Prepare ffffailed: "  . $stmt->errno . " " . $stmt->error;
}
